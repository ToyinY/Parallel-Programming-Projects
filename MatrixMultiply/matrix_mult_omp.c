...
int main () {
	...
	int num_procs, tid, chunk;
	// start timer
	gettimeofday(&tvalBefore, NULL);

	#pragma omp parallel private(tid, i, j, k) shared(a, b, c)
	{
		tid = omp_get_thread_num();
		num_procs = omp_get_num_procs();
        chunk = M1_ROWS / num_procs;

		if (tid == 0) {
			printf("\nNumber of procs/threads = %d\n", num_procs);
			printf("Chunk size = %d\n", chunk);
		}
	
		printf("Thread %d is multiplying with chunk size = %d.\n", tid, chunk);
